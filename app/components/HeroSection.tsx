...
export default function HeroSection() {
    ...
    const handleFullscreen = async () => {
        const target = frameRef.current ?? gameRef.current;
        if (!target || !target.requestFullscreen) return;

        try {
            await target.requestFullscreen();
        } catch {
            window.open('/play/game', '_blank', 'noopener,noreferrer');
        }
    };
    ...
    const handleShare = async () => {
        ...
        if (navigator.share) {
            await navigator.share({ title: 'FillWords', text: 'Play FillWords - the daily word puzzle!', url });
            return;
        }
        ...
        <section className="hero hero-full hero-with-game" ref={heroRef} id="game">
            ...
            <div className="hero-content hero-content-wide">
                ...
                <div className="hero-cta" style={{
                    ...
                    <button type="button" onClick={handleFullscreen} className="btn btn-hero" style={{
                        display: 'inline-flex',
                        alignItems: 'center',
                        gap: '0.5rem',
                    ...
                    <button type="button" onClick={handleShare} className="btn btn-secondary btn-hero share-btn-hero" style={{
                        display: 'inline-flex',
                        alignItems: 'center',
                        gap: '0.5rem',
                        padding: '0.65rem 1.5rem',
